import { pgTable, integer, jsonb, timestamp, text, uniqueIndex } from "drizzle-orm/pg-core";
 ...
 * tenant's text slug; uniqueness per tenant is enforced by a partial
 * unique index. The `id` column remains the PK to preserve backward
 * compat with any code still hard-coding `WHERE id = 1`.
 *
 * `tenant_id` is `text` to match the live DB type (no destructive ALTER
 * required).
 */
